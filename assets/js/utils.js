...
function createPolygonCoordinates(map, centerLng, centerLat, sides = 6, sizeMultiplier = 1.0) {
    ...
    const radiusInMeters = Math.sqrt(targetAreaInSquareMeters / ((3 * Math.sqrt(3) / 2))) * sizeMultiplier;
    
    console.log(`Creating polygon with ${radiusInMeters.toFixed(2)}m radius to cover ${targetAreaInSquareMeters}m² area (${targetAreaInSquareMeters/1000000} km²)`);
...
function ensureMapLayers(map) {
    ...
    try {
        ...
        if (map.getLayer('location-polygons') && map.getLayer('location-polygons-outline') && map.getSource('locations')) {
            return true;
        }
        ...
        return map.getLayer('location-polygons') && 
